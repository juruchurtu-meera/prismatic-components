...
export const paginateRecordsWithCursor = async <T>(
  ...
  if (!fetchAll) {
    const { additional_data, data } = await fetchData<
      GenericCursorPaginationData<T>
    >(client, path, params);
    return {
      data,
      additional_data,
  ...
  do {
    const { data, additional_data } = await fetchData<
      GenericCursorPaginationData<T>
    >(client, path, {
      ...params,
      limit: PAGINATION_LIMIT,
...
export const validateConnection = (connection: Connection) => {
  ...
  if (!connectionsMap.includes(connection.key)) {
    throw new ConnectionError(
      connection,
      `Connection key is not valid: ${connection.key}`,
    );
  }
};
...
export const getBaseUrl = (version: string) => {
  const baseUrl =
    version === "v1" ? `${BASE_URL}/${version}` : `${BASE_URL}/api/${version}`;
  return baseUrl;
};
...
export const partitionByTimestamp = (
  ...
  created: PipedriveRecentItem[];
  updated: PipedriveRecentItem[];
} => {
