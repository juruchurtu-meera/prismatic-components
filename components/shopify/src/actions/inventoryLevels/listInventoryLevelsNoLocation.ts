...
import {
  inventoryItemIds,
  listInventoryLevelsNoLocationInputs,
  locationIds,
} from "../../inputs";
...
export const listInventoryLevelsNoLocation = action({
  display: {
    label: "List Inventory Levels",
    description: "Lists all inventory levels.",
  },
  perform: async (context, params) => {
    ...
    if (!params.inventoryItemIds && !params.locationIds)
      throw new Error(
        `Either ${inventoryItemIds.label} or ${locationIds.label} must be provided`,
      );
