...
export const pollChangesTrigger = pollingTrigger({
  ...
  perform: async (
    ...
    if (resourceType === PollResource.REPORT_SUITES) {
      if (!globalCompanyId) {
        throw new Error(
          "Global Company ID is required when polling report suites.",
        );
      }
      ...
      records = data.imsOrgs.flatMap(
        (org) => org.companies,
      ) as unknown as AdobeRecord[];
    }
